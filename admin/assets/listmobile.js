...
function formMobileTableData(mobiles){
    ...
    for(let mobileObj of mobiles){
        ...
        <img src="${imageUrl}" alt="${imageUrl}" width="300px" height="150px" />${mobileObj.brandName}</td><td>${mobileObj.modelName}</td><td>Rs.${mobileObj.modelPrice}</td><td>${mobileObj.modelConfiguration}</td><td><a href='edit.html?id=${mobileObj._id}'>Edit</a></td><td>${mobileObj.status}</td><td><button onClick="deleteMobile('${mobileObj._id}','${mobileObj._rev}')">Delete</button></td></tr>`;
    }
    console.log(content);
